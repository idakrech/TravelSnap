...
const AddPostPage = () => {
  ...
  const {params: {photoUri}} = useRoute<AddPostScreenRouteProp>();
  ...
  return (
    <LinearGradient style={{flex: 1, alignItems: "center", padding: 16}} colors={['#ffc0a0', '#ffe7a0']}>
      <Image source={{uri: photoUri}} style={{width: "90%", aspectRatio: 1, marginTop: 32, marginBottom: 16, borderRadius: 12}}/>
      <Text>{cityName}</Text>
      <Input onInputChange={setCaption} placeholderText='Add caption'/>
      <ButtonGradient label={loading ? "Uploading..." : "Upload"} onPress={uploadPost}/>
    </LinearGradient>
  )
}
